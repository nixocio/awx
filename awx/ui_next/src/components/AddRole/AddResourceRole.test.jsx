...
describe('<AddResourceRole />', () => {
  ...
  test('initially renders without crashing', () => {
    ...
    expect(wrapper.find('AddResourceRole').length).toBe(1);
  });
  ...
  test('should successfully click user/team cards', async () => {
    let wrapper;
    act(() => {
      wrapper = mountWithContexts(
        <AddResourceRole onClose={() => {}} onSave={() => {}} roles={roles} />,
  ...
  test.only('should reset values with resource type changes', async () => {
    let wrapper;
    act(() => {
      wrapper = mountWithContexts(
        <AddResourceRole onClose={() => {}} onSave={() => {}} roles={roles} />,
  ...
  test('should not display team as a choice in case credential does not have organization', () => {
    ...
    expect(selectableCardWrapper.prop('label')).toBe('Users');
  });
});
