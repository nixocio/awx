import { yamlToJson, jsonToYaml, parseVariableFields } from './yaml';
...
`;
  ...
  test('should convert empty yaml to an object', () => {
    expect(parseVariableFields('---')).toEqual({});
  });
  ...
  test('should convert yaml string to json object', () => {
    expect(parseVariableFields(injector)).toEqual(
      JSON.parse(yamlToJson(injector))
    );
  });

  test('should convert json string to json object', () => {
    expect(parseVariableFields(input)).toEqual(JSON.parse(input));
  });
});
